...
$filedata = ConvertFrom-StringData (Get-Content .\NetStats.properties -Raw)

$wait = $filedata.waitTime
$passArgs = $filedata.passArgs
...
if($args -ne 0)
{
$OUT = Get-NetworkStatistics $args
}
if($passArgs)
{
$OUT = Get-NetworkStatistics $passArgs
}
else
{
$OUT = Get-NetworkStatistics
}

foreach($LINE in $OUT)
{
$LOG = $TIME+",{ `"Protocol`":`""+$LINE.Protocol+ "`",`"LocalAddress`":`""+$LINE.LocalAddress+"`",`"LocalPort`":`""+$LINE.LocalPort+"`",`"RemoteAddress`":`""+$LINE.RemoteAddress+"`",`"RemotePort`":`""+$LINE.RemotePort+"`",`"State`":`""+$LINE.State+"`",`"ProcessName`":`""+$LINE.ProcessName+"`",`"PID`":`""+$LINE.Pid+"`" }"
$LOG
}
sleep $wait
}
